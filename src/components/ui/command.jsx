...
const CommandInput = React.forwardRef(({ className, ...props }, ref) => (
  <div>
    <div
      className="flex items-center text-[] w-full px-3"
      cmdk-input-wrapper=""
    >
      <Search className="h-5 w-5 shrink-0  text-[#BDBDBD] mr-2" />
      <CommandPrimitive.Input
        ref={ref}
        className={cn(
          "flex h-9 w-full rounded-md bg-transparent py-3 text-sm outline-hidden placeholder:text-slate-[#BDBDBD] disabled:cursor-not-allowed disabled:opacity-50 dark:placeholder:text-slate-400",
          className,
        )}
        {...props}
      />
...
const CommandList = React.forwardRef(({ className, ...props }, ref) => (
  <ScrollArea className="overflow-y-auto overflow-x-hidden m  x-2">
...
const CommandItem = React.forwardRef(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-hidden data-[disabled=true]:pointer-events-none data-[selected=true]:bg-slate-100 data-[selected=true]:text-slate-900 data-[disabled=true]:opacity-50 dark:data-[selected=true]:bg-slate-800 dark:data-[selected=true]:text-slate-50",
      className,
    )}
    {...props}
  />
